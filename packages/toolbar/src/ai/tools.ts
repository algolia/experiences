  ...
  for (const [key, value] of Object.entries(parameters)) {
    if (key === 'cssVariables' && typeof value === 'object' && value !== null) {
      for (const [cssKey, cssValue] of Object.entries(
        ...
        callbacks.onCssVariableChange(cssKey, cssValue);
...
- CSS variable theming is managed globally via the Theme tab, not per-widget. To edit CSS variables via edit_widget, pass \`cssVariables\` in \`parameters\` as key-value pairs (e.g. \`{"cssVariables": {"primary-color-rgb": "255, 0, 0"}}\`). These update the global theme.
